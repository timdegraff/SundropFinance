...
export default function App() {
  ...
  // --- Handlers for Smart Tables (CRUD) ---
  const handleLineItemUpdate = (type: 'revenue' | 'budget', id: string, field: 'modifierPercent' | 'modifierFixed' | 'finalValue' | 'baseline' | 'label', value: any) => {
    setState(prev => {
        const list = type === 'revenue' ? prev.revenueItems : prev.budgetItems;
        const updatedList = list.map(item => {
            if (item.id !== id) return item;
            if (field === 'label') return { ...item, label: value };
            if (field === 'baseline') return { ...item, baseline: value };
            if (field === 'finalValue') {
            ...
            if (field === 'modifierPercent') return { ...item, modifierPercent: value, modifierFixed: 0 };
  ...
  const handleAddLineItem = (type: 'revenue' | 'budget') => {
    ...
    setState(prev => ({
      ...prev,
      [type === 'revenue' ? 'revenueItems' : 'budgetItems']: [...(type === 'revenue' ? prev.revenueItems : prev.budgetItems), newItem]
    }));
  };

  const handleDeleteLineItem = (type: 'revenue' | 'budget', id: string) => {
    setState(prev => ({
      ...prev,
      [type === 'revenue' ? 'revenueItems' : 'budgetItems']: (type === 'revenue' ? prev.revenueItems : prev.budgetItems).filter(i => i.id !== id)
    }));
  };
  ...
  // Specific extraction of Afterschool for pie chart as requested
  const afterschoolItem = financials.processedRevenue.find(i => i.id === 'r_afterschool');
  const afterschoolVal = afterschoolItem?.finalValue || 0;

  const otherRevenue = financials.processedRevenue
    .filter(i => i.id !== 'tuition' && i.id !== 'r_afterschool')
  ...
  // Render logic remains identical except for passed down handlers
  if (!user) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleLogin} className="w-full bg-white text-slate-900 font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-3 hover:bg-slate-100">
            ...
            <div className="pt-4 mt-2 border-t border-slate-800 text-center">
                <button onClick={handleOfflineLogin} className="text-[10px] text-amber-500 hover:text-amber-400 underline uppercase tracking-wide opacity-80 hover:opacity-100">
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center space-x-1 bg-slate-800/50 rounded-full p-1">
            {[CARDS.STRATEGY, CARDS.REVENUE, CARDS.BUDGET, CARDS.MONTHLY].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${activeTab === tab ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'} uppercase tracking-wide`}>
                {tab}
              </button>
            ))}
          </nav>
          <div className="flex items-center gap-4">
             ...
             <div className="flex items-center space-x-1 border-l border-slate-700 pl-4">
                 <button onClick={handleSave} className={`px-3 py-1 text-[10px] font-bold rounded flex items-center gap-2 ${isSaving ? 'bg-amber-500/20 text-amber-500 animate-pulse' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>
                            ...
                            <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                                <div className="md:col-span-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="number" value={state.tuition.baseFTPrice} onChange={(e) => handleBasePriceChange(parseFloat(e.target.value) || 0)} className="w-full bg-slate-900 text-3xl font-bold text-white pl-8 pr-4 py-2 rounded border border-slate-700 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none" />
                                    </div>
                                </div>
                                    ...
                                    <div key={tier.id} className="p-4 bg-slate-800/50 rounded-lg border border-slate-700/50 hover:border-slate-600 transition-colors">
                                        <div className="flex justify-between items-center mb-3">
                                            ...
                                            <div className="flex items-center bg-slate-900 rounded p-1 border border-slate-700">
                                                ...
                                                <input type="number" value={tier.qty} onChange={(e) => handleTierQtyChange(tier.id, parseInt(e.target.value) || 0)} className="w-12 bg-slate-950 rounded px-1 py-0.5 text-right text-white focus:border-amber-500 outline-none text-sm" />
                                            </div>
                                        </div>
                                        <div className="flex justify-between items-center gap-2">
                                            {tier.id !== 'tuitionFT' ? (
                                                <div className="relative w-24">
                                                    <input type="number" value={Math.round(tier.ratio)} onChange={(e) => handleTierPriceOrRatioChange(tier.id, 'ratio', parseFloat(e.target.value) || 0)} className="peer w-full bg-slate-900 border border-slate-700 rounded pl-2 pr-6 py-1 text-right text-xs text-slate-400 focus:text-white focus:border-amber-500 outline-none" />
                                            ...
                                            <div className="relative flex-1">
                                                ...
                                                <input type="number" value={Math.round(tier.calculatedPrice)} onChange={(e) => handleTierPriceOrRatioChange(tier.id, 'price', parseFloat(e.target.value) || 0)} className={`w-full bg-slate-900 border border-slate-700 rounded pl-5 pr-2 py-1 text-right font-bold text-teal-400 focus:border-amber-500 outline-none ${tier.id === 'tuitionFT' ? 'opacity-50 pointer-events-none' : ''}`} readOnly={tier.id === 'tuitionFT'} />
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                    ...
                                    {financials.processedDiscounts.map((disc) => (
                                        <tr key={disc.id} className="border-b border-slate-800">
                                            ...
                                            <td className="px-4 py-3 text-center">
                                                <input type="number" value={disc.qty} onChange={(e) => handleDiscountChange(disc.id, 'qty', parseInt(e.target.value) || 0)} className="w-16 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-center text-white focus:border-amber-500 outline-none" />
                                            </td>
                                            <td className="px-4 py-3 text-center">
                                                <div className="relative inline-block w-20">
                                                    <input type="number" value={disc.discountPercent} onChange={(e) => handleDiscountChange(disc.id, 'discountPercent', parseFloat(e.target.value) || 0)} className="peer w-full bg-slate-950 border border-slate-700 rounded pl-2 pr-6 py-1 text-right text-white focus:border-amber-500 outline-none" />
                    ...
                    <div className="p-6 rounded-xl bg-slate-900 border border-slate-800 h-full">
                        ...
                        <div className="h-64 w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={revenuePieData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" startAngle={90} endAngle={-270}>
                                        {revenuePieData.map((entry, index) => (
                                            <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                                        ))}
                                    </Pie>
                                    <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', color: '#fff' }} itemStyle={{ color: '#fff' }} formatter={(value: number) => `$${value.toLocaleString()}`} />
                                </PieChart>
                            </ResponsiveContainer>
                        </div>
        ...
        {activeTab === CARDS.REVENUE && (
           <div className="space-y-6 animate-fade-in">
               ...
               <SmartTable 
                 ...
                 onAdd={() => handleAddLineItem('revenue')}
                 onDelete={(id) => handleDeleteLineItem('revenue', id)}
                 readOnlyIds={['tuition']}
               />
           </div>
        )}
        ...
        {activeTab === CARDS.BUDGET && (
            <div className="space-y-6 animate-fade-in">
               ...
               <SmartTable 
                 ...
                 onAdd={() => handleAddLineItem('budget')}
                 onDelete={(id) => handleDeleteLineItem('budget', id)}
               />
           </div>
        )}

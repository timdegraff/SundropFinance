...
import { SunIcon, TrendingUpIcon, PDFIcon } from "./components/Icons.tsx";
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <h1 className="text-2xl font-black text-center text-white mb-2">SUNDROP FINANCE</h1>
  ...
  return (
    <div className="min-h-screen pb-20 bg-[#0a0f1d] print:bg-white print:text-black">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8 space-y-12">
        {/* Strategy Section */}
        <div className={`${activeTab === 'strategy' ? 'block' : 'hidden'} print-section space-y-10`}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[
              ...
              <div key={idx} className="p-5 md:p-6 rounded-2xl bg-slate-900/40 border border-slate-800/60 backdrop-blur-sm shadow-xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
              <section className="bg-slate-900/40 border border-slate-800/60 rounded-3xl p-6 md:p-8 relative overflow-hidden shadow-2xl">
                ...
                <div className="space-y-10">
                  <div className="bg-slate-950/80 p-6 md:p-8 rounded-2xl border border-slate-800 flex flex-col md:flex-row gap-6 md:gap-8 items-center justify-between">
                    <div className="flex-1 w-full">
                      <label className="text-[10px] text-amber-500 font-black uppercase tracking-widest mb-3 block">Base Full-Time Rate (Yearly)</label>
                      <div className="relative">
                        <span className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 text-3xl">$</span>
                        <input type="number" value={state.tuition.baseFTPrice} onChange={(e) => setState(s => ({...s, tuition: {...s.tuition, baseFTPrice: parseFloat(e.target.value)||0}}))} className="w-full bg-slate-900 border-2 border-slate-800 text-4xl md:text-5xl font-black text-white pl-12 pr-6 py-4 rounded-xl focus:border-amber-500 outline-none" />
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    {financials.tiers.map((tier) => (
                      <div key={tier.id} className="p-6 bg-slate-900/60 rounded-2xl border border-slate-800/60 hover:bg-slate-800/40 transition-all">
                          ...
                          <div>
                            ...
                            <div className="mt-2 flex items-center gap-2">
                               <input 
                                 ...
                                 className="w-12 bg-slate-950 border border-slate-800 rounded px-1.5 py-0.5 text-[10px] font-black text-amber-500 text-center outline-none"
                               />
                        ...
                        <div className="bg-slate-950 rounded-xl p-4 flex justify-between items-center border border-slate-800">
                          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Enrolled Qty</span>
                          <input type="number" value={tier.qty} onChange={(e) => setState(s => ({...s, tuition: {...s.tuition, tiers: {...s.tuition.tiers, [tier.id]: {...tier, qty: parseInt(e.target.value)||0}} }}))} className="w-16 bg-slate-900 border border-slate-800 text-center font-black text-white py-1.5 rounded-lg outline-none focus:border-amber-500" />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Discounts Section */}
              <section className="bg-slate-900/40 border border-slate-800/60 rounded-3xl p-6 md:p-8 shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {financials.processedDiscounts.map(d => (
                    <div key={d.id} className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-4 shadow-lg">
                      <div className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em]">{d.label}</div>
                      <div className="flex justify-between items-center border-y border-slate-800/50 py-3">
                        <div className="text-center">
                          <div className="text-[9px] text-slate-600 font-bold uppercase mb-1">Students</div>
                          <input type="number" value={d.qty} onChange={(e) => setState(s => ({...s, discounts: {...s.discounts, [d.id]: {...d, qty: parseInt(e.target.value) || 0}} }))} className="bg-slate-900 border border-slate-800 w-10 text-center rounded text-xs font-black p-0.5 text-white" />
                        </div>
                        <div className="text-center">
                          <div className="text-[9px] text-slate-600 font-bold uppercase mb-1">Disc %</div>
                          <input type="number" value={d.discountPercent} onChange={(e) => setState(s => ({...s, discounts: {...s.discounts, [d.id]: {...d, discountPercent: parseFloat(e.target.value) || 0}} }))} className="bg-slate-900 border border-slate-800 w-10 text-center rounded text-xs font-black p-0.5 text-white" />
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-sm font-black text-rose-500">-${Math.round(d.discountValue).toLocaleString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-8">
              <div className="bg-slate-900/40 border border-slate-800/60 rounded-3xl p-6 md:p-8 flex flex-col shadow-2xl">
                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-10">Revenue Composition</h3>
                <div className="relative h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={revenuePieData} cx="50%" cy="50%" innerRadius={70} outerRadius={95} paddingAngle={0} dataKey="value" stroke="#0a0f1d" strokeWidth={5}>
                  ...
                  {revenuePieData.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between group">
                        ...
                        <div className="text-right">
                          <span className="text-xs font-mono font-bold text-slate-200 mr-4">${Math.round(item.value/1000).toLocaleString()}K</span>
        ...
        {/* Revenue Section */}
        <div className={`${activeTab === 'revenue' ? 'block' : 'hidden'} print-section space-y-6`}>
        ...
        {/* Budget Section */}
        <div className={`${activeTab === 'budget' ? 'block' : 'hidden'} print-section space-y-6`}>
      ...
      <footer className="fixed bottom-0 left-0 w-full bg-slate-950/90 backdrop-blur-xl border-t border-slate-900 py-3 px-6 text-[9px] font-black text-slate-600 flex justify-between tracking-[0.2em] uppercase z-40 no-print">
        <div className="flex items-center gap-4">
            <span className="text-slate-500">Sundrop Finance v27.6</span>
